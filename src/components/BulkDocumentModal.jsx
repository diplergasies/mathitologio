...
export default function BulkDocumentModal({ ids, onClose }) {
  ...
  useEffect(() => {
    api.listTemplates().then((t) => {
      const list = t || []
      setTemplates(list)
      setSelected(list.map((x) => x.file)) // προεπιλογή: όλα τα templates
    })
  }, [])
  ...
  async function generate() {
    setBusy(true)
    setResult(null)
    const res = await api.bulkGenerate(ids, selected)
    setBusy(false)
    ...
    <>
      ...
      {!result && (
        ...
        >
          ...
          {busy ? 'Δημιουργία…' : `Έκδοση για ${ids.length} μαθητές`}
        </button>
      )}
    </>
  )
  ...
  return (
    <Modal title={`Μαζική έκδοση εγγράφων (${ids.length} μαθητές)`} onClose={onClose} footer={footer}>
      {templates.length === 0 ? (
        ...
        <>
          ...
          <div className="space-y-1">
            {templates.map((t) => (
              <label key={t.file} className="flex cursor-pointer items-center gap-2 rounded-md border border-slate-200 p-2 text-sm">
          ...
          <p className="mt-2 text-xs text-slate-400">
            Θα δημιουργηθεί ένα PDF ανά μαθητή & έγγραφο σε φάκελο που θα επιλέξεις
            ({ids.length} × {selected.length} = {ids.length * selected.length} αρχεία).
          </p>
        </>
      )}
